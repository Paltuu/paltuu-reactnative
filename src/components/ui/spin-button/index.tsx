...
const SpinButton: React.FC<SpinButtonProps> = ({
  ...
  // Drives the color/spinner animation toward `newState`. Shared by the press
  // handler (uncontrolled + controlled taps) and the controlled-sync effect
  // below, so external state changes (e.g. an async upload finishing) animate
  // the button back to idle even when the user didn't tap it again.
  ...
  // Controlled mode: react to the external `isActive` prop changing on its
  // own (no press involved), e.g. when an async action finishes and flips
  // it back to false — without this the button would stay stuck mid-animation.
  useEffect(() => {
    if (!controlled) return;
    animateToState(isActive ?? false);
  }, [controlled, isActive, animateToState]);
          ...
          style={[
            {
              ...
              fontFamily: mergedButtonStyle.fontFamily,
            },
            animatedTextStyle,
          ]}
