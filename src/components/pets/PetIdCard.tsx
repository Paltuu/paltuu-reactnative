...
export const PetIdCard = ({
    ...
    const identityNumber = isPlaceholder
        ? DASH
        : pet
          ? formatPetIdentityNumber(pet.pet_profile_id)
          : DASH;
    const dob = isPlaceholder
        ? DASH
        : pet?.date_of_birth
          ? formatCardDate(pet.date_of_birth)
          : DASH;
    const issue = isPlaceholder
        ? DASH
        : pet?.created_at
          ? formatCardDate(pet.created_at)
          : DASH;
    const expiry = isPlaceholder
        ? DASH
        : pet?.created_at
          ? formatCardExpiry(pet.created_at)
          : DASH;
...
const HEADER_RADIUS = CARD_RADIUS - CARD_BORDER_WIDTH; // 16.5 — nests exactly inside the card's own curve
