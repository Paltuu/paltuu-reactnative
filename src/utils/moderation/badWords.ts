 ...
 * warning while composing a post/comment (see useBadWordsWarning below and
 * its usage in create-post.tsx / CommentComposer.tsx). It is NOT a security
 * boundary: it never blocks a submission, and the server re-checks the same
 * words authoritatively and is the only thing that can actually shadow-hide
 * content — see paltuu-nextjs/petproj/lib/moderation/badWords.ts, which is
 * the canonical copy. Keep the two in sync manually; they're small enough
 * that a shared package wasn't worth the cross-repo wiring.
 *
 * See the Next.js copy for the full rationale on the SEVERE/MILD split and
 * why common pet-app words ("bitch", "kutta", "gadha", "suar", "pilla",
 * "ullu", "sex", "gandi", "dalal"...) are deliberately kept out of SEVERE.
 */
