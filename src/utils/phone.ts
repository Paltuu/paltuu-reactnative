/**
 * Phone-number helpers, kept in sync with the web app's `utils/phone.ts`.
 *
 ...
 * "+971501234567"). The website's create-listing and adoption forms now let
 * users pick a country code, so listings and applications can carry non-PK
 * numbers — the app needs to display and edit those without mangling them.
 *
 * `normalizePhone` is intentionally forgiving: legacy "+92"-only payloads keep
 * working, and only genuinely unusable input returns null.
 *
 * NOTE: the older PK-only helpers (`isValidPkPhone`, `normalizeIncomingPhone`,
 * `formatDisplayPhone`) still live in `src/components/ui/PhoneInput.tsx` and are
 * used by the Karachi-only express-vet flows. This module is the international
 * superset used by the pet-listing and adoption flows.
 */
 ...
 *
 *  "+923001234567"      -> "+923001234567"   (pass-through)
...
export function normalizePhone(raw: string | null | undefined): string | null {
  ...
  if (s.startsWith('+')) {
    ...
    return digits.length >= 7 && digits.length <= 15 && digits[0] !== '0' ? `+${digits}` : null;
  }
  ...
  if (normalized) {
    ...
    for (const dial of KNOWN_DIALS) {
      if (digits.startsWith(dial)) {
        ...
        const country =
          COUNTRY_DIALS.find(
            (c) => c.dial === dial && national.length >= c.min && national.length <= c.max,
 ...
 * plausible length for the detected country.
 */
...
export function formatPhoneDisplay(value: string | null | undefined): {
  ...
  let pretty = raw;
  if (known) {
    const grouped = national.length > 6 ? `${national.slice(0, 3)} ${national.slice(3)}` : national;
